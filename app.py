...
for list_item in daily_transactions_split:
  sales_item = []
  for item in list_item:
    sales_item.append(item.replace("\n", "").strip())
  transactions_clean.append(sales_item)
...
# 19, 20
def color_count(item):
  return thread_sold_split.count(item)

# print(color_count("white"))    // returns 28
...
# 22
for item in colors:
  message = 'Today we sold {amount} {color} colour threads.'.format(amount = color_count(item), color = item)
  print(message)